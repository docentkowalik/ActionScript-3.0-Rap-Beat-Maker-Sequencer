package  {
	...
	public class CrashButton extends MovieClip {
		...
		public var crashSoundArray:Array = new Array();	// array variable that will hold the sound of the button
														// when the button is pressed, alpha is less than 1 and 
														//then the sound will be pushed into the array


		
		[Embed(source = "sounds/Clave.mp3")]	//getting the path of the sample and embeding it in the app
		
		public var crashSoundClass:Class;
		
		public var crashSound:Sound = new crashSoundClass() as Sound;
		
		public function CrashButton()
		{
			// constructor code
		...
		public function checkButtonState(event:MouseEvent)
		{
			if (this.alpha == 1)	//by default the alpha value is equal to 1, so if it is one
			{
				this.alpha = 0.5;
				//crashSound.play();
				crashSoundArray.push(crashSound);
				trace(crashSoundArray);
